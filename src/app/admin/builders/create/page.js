...
import { MdArrowBack, MdImage, MdAdd, MdDelete, MdExpandMore } from 'react-icons/md';
...
export default function CreateBuilderPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (result.success) {
                await Swal.fire({ icon: 'success', title: 'Builder Created!', timer: 2000, showConfirmButton: false });
                router.push('/admin/builders');
            } else {
                await Swal.fire({ icon: 'error', title: 'Error', text: result.error });
            }
        } catch {
            await Swal.fire({ icon: 'error', title: 'Error', text: 'Failed to create builder' });
        } finally {
            setSubmitting(false);
        }
    };
            ...
            <main className={`flex-1 ${sidebarOpen ? 'ml-64' : 'ml-20'} transition-all duration-300`}>
                <div className="p-8">
                    <div className="mb-8">
                        ...
                        <h1 className="text-3xl font-bold text-gray-800">Add New Builder</h1>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6 max-w-3xl">
                        ...
                        <div className="flex gap-4 pb-10">
                            <button type="submit" disabled={submitting}
                                ...
                                {submitting ? 'Saving...' : 'Create Builder'}
                            </button>
